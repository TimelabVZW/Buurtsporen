...
const SVGButton: React.FC<SVGButtonProps> = (props: SVGButtonProps) => {
  return (
    <button className={props.className? props.className : props.small? 'button--svg button--svg--small': 'button--svg'} onClick={props.onClick} title={props.title? props.title : ''}>
        {props.children}
    </button>
  );
};
